...
void SPI2_TransmitByte(uint8_t data)
{
    // Keo chan NSS xuong de bat dau quá trình truyen
    //LL_GPIO_ResetOutputPin(GPIOB, LL_GPIO_PIN_4); // NSS la chan PB4
    
    // Doi cho toi khi TX buffer trong (TXE flag duoc set)
    ...
    //LL_GPIO_SetOutputPin(GPIOB, LL_GPIO_PIN_4);  // NSS là chân PB4
}
...
uint8_t SPI2_ReceiveByte(void)
{
    ...
    // Keo chan NSS xuong de bat dau quá trình nhan
    //LL_GPIO_ResetOutputPin(GPIOB, LL_GPIO_PIN_4); // NSS la chan PB4
    ...
    // Doi cho toi khi du lieu duoc nhan ve (RXNE flag duoc set)
    while (!LL_SPI_IsActiveFlag_RXNE(SPI2));

    // Doc du lieu nhan tu SPI
    receivedData = LL_SPI_ReceiveData8(SPI2);

    // Keo chan NSS len de ket thuc qua trinh nhan
    //LL_GPIO_SetOutputPin(GPIOB, LL_GPIO_PIN_4);  // NSS la chan PB4
