...
volatile uint8_t I2C_SendByteRetAddr = 0;
volatile uint8_t I2C_SendByteRetData = 0;
volatile uint32_t failedAddrCounter = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
		...
		I2C_SendByteRetAddr = I2C_SendByte(SLAVE_ADRESSE << 1);
		delay_ms(1000);
    /* USER CODE END WHILE */
