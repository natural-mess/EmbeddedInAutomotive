...
#define FCLK_FAST() { MODIFY_REG(SPI2->CR1, SPI_CR1_BR, LL_SPI_BAUDRATEPRESCALER_DIV32);}	/* Set SCLK = fast, approx 2 MBits/s */
...
void SPI_Timer_On(uint32_t waitTicks) {
    spiTimerTickStart = LL_TIM_GetCounter(TIM2);
    spiTimerTickDelay = waitTicks;
}

uint8_t SPI_Timer_Status() {
		uint32_t currentTicks = LL_TIM_GetCounter(TIM2);
...
)
{
    ...
    //HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, &dat, &rxDat, 1, 50);
		...
		LL_SPI_TransmitReceive(SPI2, &dat, &rxDat, 1, 50);
    return rxDat;
}
...
)
{
  ...
  uint32_t waitSpiTimerTickDelay;
	...
	waitSpiTimerTickStart = LL_TIM_GetCounter(TIM2);
	waitSpiTimerTickDelay = (uint32_t)wt;
	do {
		d = xchg_spi(0xFF);
		/* This loop takes a time. Insert rot_rdq() here for multitask envilonment. */
	} while (d != 0xFF && ((LL_TIM_GetCounter(TIM2) - waitSpiTimerTickStart) < waitSpiTimerTickDelay));	/* Wait for card goes ready or timeout */
