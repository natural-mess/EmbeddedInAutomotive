...
#define FCLK_SLOW() { MODIFY_REG(SPI2->CR1, SPI_CR1_BR, LL_SPI_BAUDRATEPRESCALER_DIV128); }	/* Set SCLK = slow, approx 280 KBits/s*/
#define FCLK_FAST() { MODIFY_REG(SPI2->CR1, SPI_CR1_BR, LL_SPI_BAUDRATEPRESCALER_DIV4); }	/* Set SCLK = fast, approx 4.5 MBits/s */

#define CS_HIGH()	{LL_GPIO_SetOutputPin(GPIOB, LL_GPIO_PIN_1);}
#define CS_LOW()	{LL_GPIO_ResetOutputPin(GPIOB, LL_GPIO_PIN_1);}
...
void SPI_Timer_On(uint32_t waitTicks) {
    spiTimerTickStart = LL_TIM_GetCounter(TIM1);
    spiTimerTickDelay = waitTicks;
}

uint8_t SPI_Timer_Status() {
		uint32_t currentTicks = LL_TIM_GetCounter(TIM1);
    return ((currentTicks - spiTimerTickStart) < spiTimerTickDelay);
}
...
)
{
    ...
    //HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, &dat, &rxDat, 1, 50);
	  SPI2_TransmitByte(dat);
		rxDat = SPI2_ReceiveByte();
    return rxDat;
}
...
)
{
  ...
  uint32_t waitSpiTimerTickDelay;
	...
	waitSpiTimerTickStart = LL_TIM_GetCounter(TIM1);
	waitSpiTimerTickDelay = (uint32_t)wt;
	do {
		d = xchg_spi(0xFF);
		/* This loop takes a time. Insert rot_rdq() here for multitask envilonment. */
	} while (d != 0xFF && ((LL_TIM_GetCounter(TIM1) - waitSpiTimerTickStart) < waitSpiTimerTickDelay));	/* Wait for card goes ready or timeout */
